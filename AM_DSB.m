...
mt_fft = fft(mt);
ct = ac*cos(2*pi*fc*t); %sinal da portadora
ct_fft = fft(ct);
ka = 0.5;#Constante de sensibilidade
#ka*mt < 1 , pra evitar sobremodulação
st = ac.*[1+ka.*mt].*cos(2*pi*fc*t); %sinal modulado, o sinal de mensagem é multiplicado pela sinal da portadora
#[1+ka.*mt], esse 1 é a parcela DC
st_fft = fft(st);
